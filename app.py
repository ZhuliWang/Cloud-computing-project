...
CHANGE_SUCCESSFUL = ({"message": "Change Successful"}, 201)
...
class Index(Resource):
    @staticmethod
    def get():
        title = "This is a project of Group 28"
        return title
...
class Register(Resource):
    ...
    def post():
        ...
        if not user:
            user = User(name=name, username=username, password=password, email=email, phoneNumber=phoneNumber,
                        pincode=pincode, address=address, created=datetime.utcnow())
            db.session.add(user)
            db.session.commit()
...
class EditPassword(Resource):
    ...
    def put():
        ...
        existedPassword = request.json.get("existedPassword").strip()
        newPassword = request.json.get("newPassword").strip()

        if userIdentity is None or existedPassword is None:
            return INVALID_INPUT_422
        ...
        isPasswordMatched = bcrypt.checkpw(existedPassword.encode("utf-8"), user.password)
        if not isPasswordMatched:
            return INVALID_CREDENTIALS__422
        user.password = bcrypt.hashpw(newPassword.strip().encode("utf-8"), bcrypt.gensalt())
...
class Weather(Resource):
    ...
    def post():
        ...
        return api.print_weather(location)
...
def generate_routes(app):
    ...
    api.add_resource(DeleteBlacklist, "/deleteBlacklist")
